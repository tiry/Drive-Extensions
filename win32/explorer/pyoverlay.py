...
def getNuxeoDriveRoots():
    if (len(driveRoots) == 0):
        nxlog("Getting Nuxeo Drive local folders")
        driveRoots = [urllib.quote(x) for x in driveExec(['local_folders', ])]
    return driveRoots
...
if __name__=='__main__':
  ...
  key = win32api.RegCreateKey (win32con.HKEY_LOCAL_MACHINE, keyname)
  win32api.RegSetValue (key, None, win32con.REG_SZ, PendingIconOverlay._reg_clsid_)
